...
#ifndef RING_VEO_ADAPTIVE_PG_HPP
...
namespace ring {
        ...
        namespace veo {
            ...
            public:
                ...
                typedef query::pg_query::patterns_type patterns_type;
            ...
            private:
                ...
                void copy(const veo_adaptive_pg &o) {
                ...
                bool var_to_vector(const var_type var, const size_type i, const state_type state, std::vector<var_type> &lonely_edges) {

                    const auto &iters = m_ptr_var_iterators->at(var);
                    if(iters.size() == 1){
                        if (state == p) {
                            lonely_edges.emplace_back(var);
                        }else {
            ...
            public:
                ...
                veo_adaptive_pg() = default;

                veo_adaptive_pg(const patterns_type *patterns,
                             const std::vector<ltj_iter_type*> *iterators,
                             const var_to_iterators_type *var_iterators,
                             ring_type *r) {
                    m_ptr_patterns = patterns;
                    ...
                    for (const auto &pattern : *m_ptr_patterns) {
                        ...
                        if (pattern.subj.is_var()) {
                            var_s = (var_type) pattern.subj.var_value;
                            //size = gao_trait_type::subject(m_ptr_ring, m_ptr_iterators->at(i));
                            s = var_to_vector(var_s, i, state_type::s, lonely_edges);
                        }
                        if (pattern.edge.is_var()) {
                            var_p = (var_type) pattern.edge.var_value;
                            //size = gao_trait_type::predicate(m_ptr_ring, m_ptr_iterators->at(i));
                            p = var_to_vector(var_p, i, state_type::p, lonely_edges);
                        }
                        if (pattern.obj.is_var()) {
                            var_o = (var_type) pattern.obj.var_value;
                            //size = gao_trait_type::object(m_ptr_ring, m_ptr_iterators->at(i));
                            o = var_to_vector(var_o, i, state_type::o, lonely_edges);
                        }
                ...
                //! Copy constructor
                veo_adaptive_pg(const veo_adaptive_pg &o) {
                    copy(o);
                }

                //! Move constructor
                veo_adaptive_pg(veo_adaptive_pg &&o) {
                    *this = std::move(o);
                }

                //! Copy Operator=
                veo_adaptive_pg &operator=(const veo_adaptive_pg &o) {
                    if (this != &o) {
                        copy(o);
                    }
                    return *this;
                ...
                //! Move Operator=
                veo_adaptive_pg &operator=(veo_adaptive_pg &&o) {
                    if (this != &o) {
                        m_ptr_patterns = std::move(o.m_ptr_patterns);
